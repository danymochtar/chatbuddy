...
def opening_prompt() -> str:
    return (
        ...
        "kurung** setelah trait-nya. Tone di sini: tetap hide istilah teknis, character-based "
        "language.\n\n"
        ...
        "## ✨ Aspek Lo Ringkas\n"
        "**PENGECUALIAN dari aturan 'invisible'** — di section INI AJA, lo BOLEH pake labels "
        "teknis (nama rasi, nama shio, nama weton, tipe MBTI) sebagai badge referensi cepat. "
        "Format bullet SATU BARIS per sistem (max 1 kalimat per baris, bukan paragraf).\n\n"
        "Template yang harus di-ikutin urutannya (skip baris kalo datanya ga ada):\n"
        "- 🔢 **Angka:** ringkas karakter inti dari misi hidup + bakat bawaan + panggilan "
        "hati (misal: _Leader (1) dengan jiwa penyayang (6) & bakat mengalir bebas (5)_)\n"
        "- ♈ **Zodiak:** Sun di rasi X (kalo udah dihitung akurat), Moon di rasi Y (kalo "
        "ada), Rising di rasi Z (kalo ada) — plus karakter ringkas. Kalo cuma Sun aja yg "
        "ada, sebut Sun doang. Kalo ga ada data astrologi sama sekali, **skip baris ini**\n"
        "- 🐉 **Shio:** hewan + elemen + vibe ringkas (misal: _Babi Kayu — ramah, loyal, "
        "artistik_)\n"
        "- 🌿 **Weton:** nama weton + neptu + vibe Jawa ringkas (misal: _Rabu Legi, neptu "
        "12 — seimbang & fleksibel_)\n"
        "- 🧠 **MBTI:** tipe + 1 kalimat vibe (SKIP kalo user belum set MBTI)\n"
        "- 💼 **Karir:** role + fit ringkas (SKIP kalo user belum set karir)\n\n"
        "PENTING: kalo data suatu baris ga ada, **skip aja — jangan bilang 'data kurang' "
        "atau sejenisnya**.\n\n"
        "## 🌞 Vibe Hari Ini\n"
        ...
        "**Style:** casual 'lo/gw', hangat. **Di LUAR section ✨ Aspek Lo Ringkas**, tetap "
        "zero jargon teknis. Astrologi tetap invisible di section lain."
    )
